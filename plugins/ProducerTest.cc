...
ProducerTest::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if ( doJets_ ) {
     ...
     std::unique_ptr<std::vector<int>> JetSeedieta_edm (new std::vector<float>(vJetSeed_ieta_));
     std::unique_ptr<std::vector<int>> JetSeediphi_edm (new std::vector<float>(vJetSeed_iphi_));
